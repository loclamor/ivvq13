...
class DocumentServiceSpec extends Specification { 
	...
	void "Document service unit test : serviceCreateDocument fail malformed arguments to doc creation service"() {
		setup:
		//persist a document, check it out
				
		def title_ = "title"
		def category_ = "Some other category"
		def content_ = "my content"
		def tags_ = "a,b,c"
		def attachments_ = "attach"
		def user_ = new User(username: "alex", password: "alex", email:"a@a.c")
	...
	void "Document service unit test : serviceCreateDocument (same tags - already exist) "() {
		setup:
		//persist a document, check it out
